module Lobbyist
  module V2
    class CommunicationCampaign < Lobbyist::V2::Base
      attr_accessor :id, :company_id, :name, :description, :cost_per_postcard, :threshold, :approved_credits,
                    :recommended_credits, :minimum_roi, :approved, :accelerator, :transaction_grouping_interval, :min_days_between_mailings, :ignore_thirty_day_bucket, :status, :campaign_type,
      ...
      def self.credit_summary(id)
        create_from_response(get("/v2/campaigns/#{id}/credit-summary.json"))
      end
    end
  end
end
